...
sbatch -p all \
  ...
  --wrap="cd '${PWD}' && HF_HUB_ENABLE_HF_TRANSFER=1 PYTHONPATH=src python scripts/download_fineweb_parquet.py --repo-id HuggingFaceFW/fineweb-edu --local-dir '${OUT_DIR}' --allow-patterns 'sample/350BT/*.parquet' '**/*.parquet'"
